...
QueueHandle_t IMU_reduced_fifo;
...
void init_freertos_tasks()
{
	...
	// fifo para datos del imu reducidos, para testeo
	IMU_reduced_fifo = xQueueCreate(FIFO_SIZE,sizeof(IMU_data_reduced_t));
	if(IMU_reduced_fifo==NULL)
...
void TaskReadTemp(void *pvParameters)
{
	while (true) 
	{
		...
		vTaskDelay(pdMS_TO_TICKS(READ_TEMP_DELAY));
...
void TaskReadIMUData(void *pvParameters)
{
	...
	// Pero basicamente se crea un array de 23 datos para tomar todos los datos del IMU (IMU_snapshot)
	// Este array se le pasa a la función IMU_read() para llenarla de los datos del momento
	// por eso se llama "snapshot".
	// Luego el struct "dato" se llena con los datos del snapshot y se llena el fifo con el struct completo! 
	//
	while(true)
	{
...
// de la TASK
//
// 17apr: ojala supiera a que me referia con lo que escribí arriba, supongo que los errores me lo enseñaran.
//
void TaskBLE(void *pvParameters)
{
   IMU_data_t d;
